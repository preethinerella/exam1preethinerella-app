# -*- coding: utf-8 -*-
"""Exam1Part1_Starter(ANS).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1EJw8Em_kBbZ34ebbZjGq0NgQ3V7pCpHo
...
filename="/content/imports-85.data.xlsx"
...
# Replace NaN in 'normalized-losses' with avg_norm_loss
df["normalized-losses"] = df["normalized-losses"].replace(np.nan, avg_norm_loss)

# Confirm no NaN values in 'normalized-losses'
...
# Replace NaN in 'bore' with avg_bore
df["bore"] = df["bore"].replace(np.nan, avg_bore)

# Confirm no NaN values in 'bore'
print(f"Missing values in bore: {df['bore'].isnull().sum()}")
...
# Convert 'stroke' to numeric just in case
...
# Confirm no NaN values in 'stroke'
print(f"Missing values in stroke: {df['stroke'].isnull().sum()}")
...
# Replace NaN in 'horsepower' with avg_horsepower
df['horsepower'] = df['horsepower'].replace(np.nan, avg_horsepower)

# Confirm no NaN values in 'horsepower'
...
# Replace NaN in 'peak-rpm' with avg_peakrpm
df['peak-rpm'] = df['peak-rpm'].replace(np.nan, avg_peakrpm)

# Confirm no NaN values in 'peak-rpm'
print(f"Missing values in peak-rpm: {df['peak-rpm'].isnull().sum()}")
...
# Replace NaN in 'num-of-doors' with the most frequent value 'four'
df["num-of-doors"] = df["num-of-doors"].replace(np.nan, "four")

# Confirm no NaN values in 'num-of-doors'
print(f"Missing values in num-of-doors: {df['num-of-doors'].isnull().sum()}")
...
# Display result
df[["height", "height_normalized"]].head()
...
# Write your code below and press Shift+Enter to execute
# Create dummy variable for 'aspiration'
aspiration_dummies = pd.get_dummies(df["aspiration"], prefix="aspiration", drop_first=True)

# Concatenate with original DataFrame
df = pd.concat([df, aspiration_dummies], axis=1)

# Display the new dummy column
df[["aspiration", *aspiration_dummies.columns]].head()
